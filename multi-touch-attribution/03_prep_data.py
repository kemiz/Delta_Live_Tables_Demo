...
# MAGIC   * `path`: the list of channels that impressions for a given campaign have been delivered to a given user.
# MAGIC   * `first_interaction`: the first channel that an impression for a given campaign has been delivered to a given user.
# MAGIC   * `last_interaction`: the last channel that an impression for a given campaign has been delivered to a given user.
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TEMP VIEW user_journey_view
# MAGIC AS
# MAGIC SELECT sub2.uid AS uid
# MAGIC   ,CASE 
...
# MAGIC         , time
# MAGIC         , conversion
# MAGIC         , dense_rank() OVER (PARTITION BY uid ORDER BY time asc) as visit_order 
# MAGIC         FROM bronze
# MAGIC       ) AS sub GROUP BY sub.uid
# MAGIC     ) AS sub2;
