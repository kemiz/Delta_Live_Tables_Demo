...
# MAGIC   * Step 2: Calculate the total conversion probablity
...
# MAGIC 
# MAGIC ```P(Conversion) = (0.2 X 0.8) + (0.2 X 0.2 X 0.1) + (0.8 X 0.6) + (0.8 X 0.4 X 0.8) + (0.8 X0.4 X 0.2 X 0.1)  = 0.90```
# MAGIC 
...
# MAGIC * Run the utils notebook to gain acces to the get_params function
# MAGIC * get_params and store the relevant values in variables
# MAGIC * Set the current database so that it doesn't need to be manually specified each time it's used
...
# MAGIC 
# MAGIC In this step, we will:
# MAGIC * Define a user-defined function (UDF), `get_transition_array`, that takes a customer journey and enumerates each of the corresponding channel transitions
# MAGIC * Register the `get_transition_array` udf as a Spark UDF so that it can be utilized in Spark SQL
# MAGIC * Use `get_transition_array` to enumerate all channel transitions in a customer's journey
# MAGIC * Construct the transition probability matrix
# MAGIC * Validate that the state transition probabilities are calculated correctly
# MAGIC * Display the transition probability matrix
...
def get_transition_probability_graph(removal_state = "null"):
  '''
  This function calculates subset of the transition probability graph based on the state to exclude
...
def calculate_conversion_probability(transition_probability_pandas_df, calculated_state_conversion_probabilities, visited_states, current_state="Start"):
  '''
  This function calculates Total conversion probability based on the subset of the transition probability graph
  removal_state: channel that we want to exclude from our Transition Probability Matrix
  returns conversion probability of current state/channel 
  '''
  #we are ignoring loops in the transition graph for sake of simplicity. 
...
# MAGIC 
# MAGIC In this step, we will:
# MAGIC * Calculate the removal effect per channel
# MAGIC * Calculate conversion attribution per channel
# MAGIC * Register conversion_pandas_df as table to use SQL
# MAGIC * View channel attribution
# MAGIC * Merge channel attribution results into gold_attribution table
